...
function renderOfficeServicesAdmin(rows) {
    ...
    rows.forEach(function(row, idx) {
        ...
        let faqRowsHtml = "";
        faqs.forEach(function(f, fIdx) {
            faqRowsHtml += `<div class="service-faq-row" data-id="${row.id}" data-faq-idx="${fIdx}" style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-start;margin-bottom:8px;">
                <input type="text" value="${esc(f.q)}" data-id="${row.id}" data-faq-idx="${fIdx}" data-faq-field="q" class="service-faq-input" placeholder="السؤال" style="flex:1;min-width:180px;padding:8px;border:1px solid #ddd;border-radius:6px;">
                <input type="text" value="${esc(f.a)}" data-id="${row.id}" data-faq-idx="${fIdx}" data-faq-field="a" class="service-faq-input" placeholder="الإجابة" style="flex:2;min-width:220px;padding:8px;border:1px solid #ddd;border-radius:6px;">
                <button class="decl-delete-btn service-faq-delete" data-id="${row.id}" data-faq-idx="${fIdx}" title="حذف السؤال"><i class="fa-solid fa-trash"></i></button>
            </div>`;
        });
        ...
        html += `<div class="admin-panel" style="padding:18px;margin-bottom:15px;display:flex;gap:18px;align-items:flex-start;flex-wrap:wrap;">
            ...
            <div style="flex:1;min-width:260px;display:flex;flex-direction:column;gap:10px;">
                ${!row.slug ? '<div style="color:#c0392b;font-size:0.78rem;background:#fdecea;padding:6px 10px;border-radius:6px;"><i class="fa-solid fa-triangle-exclamation"></i> الخدمة دي مالهاش Slug - مش هتظهر ليها صفحة مستقلة ولا هتتفهرس في جوجل</div>' : ''}
                ...
                <textarea data-id="${row.id}" data-field="description" class="service-field-input" placeholder="وصف مختصر" rows="2" style="padding:10px;border:1px solid #ddd;border-radius:8px;font-family:inherit;resize:vertical;">${row.description || ""}</textarea>

                <label style="font-size:0.75rem;color:#888;">رابط الخدمة (Slug)</label>
                <input type="text" dir="ltr" value="${esc(row.slug)}" data-id="${row.id}" data-field="slug" class="service-field-input service-slug-input" placeholder="esteshara-daribeya" style="padding:8px;border:1px solid #ddd;border-radius:6px;">

                <label style="font-size:0.75rem;color:#888;">الوصف الكامل (صفحة الخدمة)</label>
                <textarea data-id="${row.id}" data-field="fullDescription" class="service-field-input" placeholder="الوصف الكامل" rows="3" style="padding:8px;border:1px solid #ddd;border-radius:6px;font-family:inherit;resize:vertical;">${row.fullDescription || ""}</textarea>

                <label style="font-size:0.75rem;color:#888;">Meta Title</label>
                <input type="text" value="${esc(row.metaTitle)}" data-id="${row.id}" data-field="metaTitle" class="service-field-input" placeholder="Meta Title" style="padding:8px;border:1px solid #ddd;border-radius:6px;">

                <label style="font-size:0.75rem;color:#888;">Meta Description</label>
                <input type="text" value="${esc(row.metaDescription)}" data-id="${row.id}" data-field="metaDescription" class="service-field-input" placeholder="Meta Description" style="padding:8px;border:1px solid #ddd;border-radius:6px;">

                <label style="font-size:0.75rem;color:#888;">Keywords (مفصولة بفاصلة)</label>
                <input type="text" value="${esc(row.keywords)}" data-id="${row.id}" data-field="keywords" class="service-field-input" placeholder="كلمة1, كلمة2" style="padding:8px;border:1px solid #ddd;border-radius:6px;">

                <div style="margin-top:8px;">
                    <label style="font-size:0.75rem;color:#888;">الأسئلة الشائعة الخاصة بالخدمة</label>
                    <div class="service-faq-list" data-id="${row.id}" style="margin-top:6px;">${faqRowsHtml}</div>
                    <button type="button" class="logout-btn service-faq-add" data-id="${row.id}" style="margin-top:6px;font-size:0.8rem;padding:6px 12px;">
    ...
    list.querySelectorAll(".service-slug-input").forEach(function(el) {
    ...
    list.querySelectorAll(".service-field-input").forEach(function(el) {
        el.addEventListener("blur", function() {
            ...
            if (field === "slug" && value) {
                db.collection("office_services").where("slug", "==", value).get().then(function(snap) {
                    const clash = snap.docs.find(d => d.id !== el.dataset.id);
                    if (clash) {
                        showToast("رابط الخدمة ده مستخدم بالفعل، اختر رابط تاني");
                        return;
                    }
                    saveServiceField(el, field, value);
                }).catch(console.error);
                return;
            }
            saveServiceField(el, field, value);
    ...
    function saveServiceField(el, field, value) {
        db.collection("office_services").doc(el.dataset.id).set({[field]: value}, {merge:true})
    ...
    list.querySelectorAll(".service-faq-add").forEach(function(btn) {
        btn.addEventListener("click", function() {
            const row = rows.find(r => r.id === btn.dataset.id);
            const faqs = Array.isArray(row.faqs) ? row.faqs.slice() : [];
            faqs.push({q: "", a: ""});
            db.collection("office_services").doc(row.id).set({faqs: faqs}, {merge:true})
                .then(function() { loadOfficeServicesAdmin(); }).catch(console.error);
        });
    });

    list.querySelectorAll(".service-faq-delete").forEach(function(btn) {
        btn.addEventListener("click", function() {
            const row = rows.find(r => r.id === btn.dataset.id);
            const faqs = Array.isArray(row.faqs) ? row.faqs.slice() : [];
            faqs.splice(parseInt(btn.dataset.faqIdx, 10), 1);
            db.collection("office_services").doc(row.id).set({faqs: faqs}, {merge:true})
                .then(function() { showToast("تم الحذف"); loadOfficeServicesAdmin(); }).catch(console.error);
        });
    });

    list.querySelectorAll(".service-faq-input").forEach(function(el) {
        el.addEventListener("blur", function() {
            const row = rows.find(r => r.id === el.dataset.id);
            const faqs = Array.isArray(row.faqs) ? row.faqs.slice() : [];
            const idx = parseInt(el.dataset.faqIdx, 10);
            if (!faqs[idx]) faqs[idx] = {q:"", a:""};
            faqs[idx][el.dataset.faqField] = el.value.trim();
            db.collection("office_services").doc(row.id).set({faqs: faqs}, {merge:true})
                .then(function() { showToast(); flashEl(el); row.faqs = faqs; }).catch(console.error);
        });
    });
}
...
window.addOfficeService = function() {
    ...
    const slugEl = document.getElementById("newServiceSlug");
    const fullDescEl = document.getElementById("newServiceFullDesc");
    const metaTitleEl = document.getElementById("newServiceMetaTitle");
    const metaDescEl = document.getElementById("newServiceMetaDesc");
    const keywordsEl = document.getElementById("newServiceKeywords");

    const name = nameEl ? nameEl.value.trim() : "";
    if (!name) { alert("اسم الخدمة مطلوب"); return; }
    if (!db) { showToast("Firebase غير متصل"); return; }

    let slug = slugEl ? slugEl.value.trim().toLowerCase().replace(/[^a-z0-9\-]/g, "-").replace(/-+/g, "-") : "";
    if (!slug) { alert("رابط الخدمة (Slug) مطلوب عشان تقدر تظهر كصفحة مستقلة"); return; }

    db.collection("office_services").where("slug", "==", slug).limit(1).get().then(function(clashSnap) {
        if (!clashSnap.empty) {
            alert("رابط الخدمة ده مستخدم بالفعل، من فضلك اختر رابط مختلف");
            return Promise.reject("slug-clash-handled");
        }
        return db.collection("office_services").orderBy("order", "desc").limit(1).get();
    }).then(function(snap) {
        ...
        return db.collection("office_services").add({
            ...
            slug: slug,
            fullDescription: fullDescEl ? fullDescEl.value.trim() : "",
            metaTitle: metaTitleEl ? metaTitleEl.value.trim() : "",
            metaDescription: metaDescEl ? metaDescEl.value.trim() : "",
        ...
        if (!docRef) return; // كان فيه تعارض في الـ slug واتوقف الطلب فوق
        ...
        if (e === "slug-clash-handled") return;
        console.error(e); showToast("خطأ أثناء الإضافة");
    });
...
function loadPublicServices() {
    ...
    db.collection("office_services").orderBy("order", "asc").get().then(function(snap) {
        ...
        snap.forEach(function(doc) {
            ...
            const cardInner = `<div class="icon-wrapper">${d.imageUrl ? `<img src="${d.imageUrl}" alt="${esc(d.name)}">` : '<i class="fa-solid fa-briefcase"></i>'}</div>
                <h3>${esc(d.name)}</h3>
                <p>${esc(d.description)}</p>`;

            if (d.slug) {
                html += `<a href="/services/${encodeURIComponent(d.slug)}" class="service-card reveal active" style="transition-delay:${delay}s;text-decoration:none;color:inherit;display:block;">${cardInner}</a>`;
            } else {
                html += `<div class="service-card reveal active" style="transition-delay:${delay}s;">${cardInner}</div>`;
            }
            i++;
        });
...
function loadBlogAdmin() {
    ...
    db.collection("blog_posts").orderBy("order", "asc").get().then(function(snap) {
        if (snap.empty) { list.innerHTML = '<div style="padding:20px;color:#999;">لا توجد مقالات مضافة بعد</div>'; return; }
        const rows = [];
        snap.forEach(d => rows.push({id: d.id, ...d.data()}));
        renderBlogAdmin(rows);
    }).catch(function(e) {
        console.error(e);
        list.innerHTML = '<div style="padding:20px;color:red;">تعذر تحميل المقالات</div>';
    });
}
...
function renderBlogAdmin(rows) {
    ...
    let html = "";
    rows.forEach(function(row, idx) {
        const missingRoute = !row.link && !row.slug;
        html += `<div class="admin-panel" style="padding:18px;margin-bottom:15px;display:flex;gap:18px;align-items:flex-start;flex-wrap:wrap;">
            <div style="display:flex;flex-direction:column;align-items:center;gap:8px;">
                <img src="${row.coverImage || ''}" alt="" style="width:80px;height:80px;border-radius:50%;object-fit:cover;background:#f0f0f0;border:1px solid #eee;">
                <label class="logout-btn" style="cursor:pointer;font-size:0.75rem;padding:6px 10px;">
                    <i class="fa-solid fa-image"></i> تغيير الصورة
                    <input type="file" accept="image/*" data-id="${row.id}" class="blog-img-input" style="display:none;">
                </label>
            </div>
            <div style="flex:1;min-width:260px;display:flex;flex-direction:column;gap:10px;">
                ${missingRoute ? '<div style="color:#c0392b;font-size:0.78rem;background:#fdecea;padding:6px 10px;border-radius:6px;"><i class="fa-solid fa-triangle-exclamation"></i> المقال ده مش هيظهر ليه صفحة ولا رابط - حط Slug أو رابط خارجي</div>' : ''}
                <input type="text" value="${esc(row.name)}" data-id="${row.id}" data-field="name" class="blog-field-input" placeholder="اسم المقال" style="font-weight:800;padding:10px;border:1px solid #ddd;border-radius:8px;">
                <input type="text" value="${esc(row.description)}" data-id="${row.id}" data-field="description" class="blog-field-input" placeholder="وصف مختصر" style="padding:10px;border:1px solid #ddd;border-radius:8px;">

                <label style="font-size:0.75rem;color:#888;">رابط خارجي (لو موجود بيتجاهل صفحة المقال الداخلية)</label>
                <input type="text" dir="ltr" value="${esc(row.link)}" data-id="${row.id}" data-field="link" class="blog-field-input" placeholder="https://..." style="padding:8px;border:1px solid #ddd;border-radius:6px;">

                <label style="font-size:0.75rem;color:#888;">رابط المقال (Slug)</label>
                <input type="text" dir="ltr" value="${esc(row.slug)}" data-id="${row.id}" data-field="slug" class="blog-field-input blog-slug-input" placeholder="tips-tax-2026" style="padding:8px;border:1px solid #ddd;border-radius:6px;">

                <label style="font-size:0.75rem;color:#888;">محتوى المقال الكامل</label>
                <textarea data-id="${row.id}" data-field="body" class="blog-field-input" placeholder="محتوى المقال" rows="3" style="padding:8px;border:1px solid #ddd;border-radius:6px;font-family:inherit;resize:vertical;">${row.body || ""}</textarea>

                <label style="font-size:0.75rem;color:#888;">Meta Title</label>
                <input type="text" value="${esc(row.metaTitle)}" data-id="${row.id}" data-field="metaTitle" class="blog-field-input" placeholder="Meta Title" style="padding:8px;border:1px solid #ddd;border-radius:6px;">

                <label style="font-size:0.75rem;color:#888;">Meta Description</label>
                <input type="text" value="${esc(row.metaDescription)}" data-id="${row.id}" data-field="metaDescription" class="blog-field-input" placeholder="Meta Description" style="padding:8px;border:1px solid #ddd;border-radius:6px;">

                <label style="font-size:0.75rem;color:#888;">Keywords (مفصولة بفاصلة)</label>
                <input type="text" value="${esc(row.keywords)}" data-id="${row.id}" data-field="keywords" class="blog-field-input" placeholder="كلمة1, كلمة2" style="padding:8px;border:1px solid #ddd;border-radius:6px;">
            </div>
            <div style="display:flex;flex-direction:column;gap:8px;">
                <button class="logout-btn blog-move-up" data-id="${row.id}" ${idx===0?"disabled":""} title="تحريك لأعلى"><i class="fa-solid fa-arrow-up"></i></button>
                <button class="logout-btn blog-move-down" data-id="${row.id}" ${idx===rows.length-1?"disabled":""} title="تحريك لأسفل"><i class="fa-solid fa-arrow-down"></i></button>
                <button class="decl-delete-btn blog-delete" data-id="${row.id}" title="حذف"><i class="fa-solid fa-trash"></i></button>
            </div>
        </div>`;
    });
    list.innerHTML = html;

    list.querySelectorAll(".blog-slug-input").forEach(function(el) {
        el.addEventListener("input", function() {
            el.value = el.value.toLowerCase().replace(/[^a-z0-9\-]/g, "-").replace(/-+/g, "-");
        });
    });

    list.querySelectorAll(".blog-field-input").forEach(function(el) {
        el.addEventListener("blur", function() {
            const field = el.dataset.field;
            const value = el.value.trim();

            if (field === "slug" && value) {
                db.collection("blog_posts").where("slug", "==", value).get().then(function(snap) {
                    const clash = snap.docs.find(d => d.id !== el.dataset.id);
                    if (clash) { showToast("رابط المقال ده مستخدم بالفعل، اختر رابط تاني"); return; }
                    saveBlogField(el, field, value);
                }).catch(console.error);
                return;
            }
            saveBlogField(el, field, value);
    ...
    function saveBlogField(el, field, value) {
        db.collection("blog_posts").doc(el.dataset.id).set({[field]: value}, {merge:true})
            .then(function() { showToast(); flashEl(el); }).catch(console.error);
    }

    list.querySelectorAll(".blog-img-input").forEach(function(el) {
        el.addEventListener("change", function() {
            const file = el.files[0];
            if (file) uploadBlogCoverImage(el.dataset.id, file);
        });
    });

    list.querySelectorAll(".blog-delete").forEach(function(btn) {
        btn.addEventListener("click", function() {
    ...
    list.querySelectorAll(".blog-move-up").forEach(function(btn) {
        btn.addEventListener("click", function() { moveBlogPost(rows, btn.dataset.id, -1); });
    });
    list.querySelectorAll(".blog-move-down").forEach(function(btn) {
        btn.addEventListener("click", function() { moveBlogPost(rows, btn.dataset.id, 1); });
    });
}

function moveBlogPost(rows, id, direction) {
    const idx = rows.findIndex(r => r.id === id);
    const swapWith = idx + direction;
    if (swapWith < 0 || swapWith >= rows.length) return;
    const a = rows[idx], b = rows[swapWith];
    const batch = db.batch();
    batch.set(db.collection("blog_posts").doc(a.id), {order: b.order}, {merge:true});
    batch.set(db.collection("blog_posts").doc(b.id), {order: a.order}, {merge:true});
    batch.commit().then(function() { loadBlogAdmin(); }).catch(console.error);
}

function uploadBlogCoverImage(id, file) {
    if (!storage) { showToast("تخزين الصور غير متاح"); return; }
    showToast("جاري رفع الصورة...");
    const ref = storage.ref().child("blog_posts/" + id + "/" + Date.now() + "_" + file.name);
    ref.put(file).then(function() {
        return ref.getDownloadURL();
    }).then(function(url) {
        return db.collection("blog_posts").doc(id).set({coverImage: url}, {merge:true});
    }).then(function() {
        showToast("تم رفع الصورة");
        loadBlogAdmin();
    }).catch(function(e) { console.error(e); showToast("تعذر رفع الصورة"); });
...
window.addBlogPost = function() {
    ...
    const imageEl = document.getElementById("newBlogImage");
    const linkEl = document.getElementById("newBlogLink");
    const slugEl = document.getElementById("newBlogSlug");
    const bodyEl = document.getElementById("newBlogBody");
    const metaTitleEl = document.getElementById("newBlogMetaTitle");
    const metaDescEl = document.getElementById("newBlogMetaDesc");
    const keywordsEl = document.getElementById("newBlogKeywords");
    ...
    if (!link && !slug) {
        alert("لازم تحط رابط خارجي أو رابط المقال (Slug) عشان المقال يبقى قابل للوصول");
        return;
    }
    ...
    function proceedToAdd() {
        db.collection("blog_posts").orderBy("order", "desc").limit(1).get().then(function(snap) {
            ...
            if (file && storage) {
                const ref = storage.ref().child("blog_posts/" + docRef.id + "/" + Date.now() + "_" + file.name);
                ref.put(file).then(function() { return ref.getDownloadURL(); })
                    .then(function(url) { return db.collection("blog_posts").doc(docRef.id).set({coverImage: url}, {merge:true}); })
                    .then(finishAddBlog).catch(function(e) { console.error(e); finishAddBlog(); });
            } else { finishAddBlog(); }
        }).catch(function(e) { console.error(e); showToast("خطأ أثناء الإضافة"); });
    }

    if (slug) {
        db.collection("blog_posts").where("slug", "==", slug).limit(1).get().then(function(clashSnap) {
            if (!clashSnap.empty) { alert("رابط المقال ده مستخدم بالفعل، من فضلك اختر رابط مختلف"); return; }
            proceedToAdd();
        }).catch(console.error);
    } else {
        proceedToAdd();
...
function loadPublicBlog() {
    ...
    db.collection("blog_posts").orderBy("order", "asc").get().then(function(snap) {
        ...
        snap.forEach(function(doc) {
            const d = doc.data();
            const delay = (i * 0.1).toFixed(1);
            const externalLink = d.link && d.link.trim();
            const internalHref = d.slug ? ("/blog/" + encodeURIComponent(d.slug)) : "";
            const href = externalLink || internalHref;
            const tag = href ? "a" : "div";
            const attrs = href ? `href="${esc(href)}"${externalLink ? ' target="_blank" rel="noreferrer"' : ""}` : "";
            html += `<${tag} ${attrs} class="service-card reveal active" style="transition-delay:${delay}s;text-decoration:none;color:inherit;display:block;">
                <div class="icon-wrapper">${d.coverImage ? `<img src="${d.coverImage}" alt="${esc(d.name)}">` : '<i class="fa-solid fa-newspaper"></i>'}</div>
                <h3>${esc(d.name)}</h3>
                <p>${esc(d.description)}</p>
            </${tag}>`;
            i++;
...
// =====================================================================
// === تتبع ضغطات واتساب و"اتصل بنا" في كل صفحات الموقع العامة ===
// بيتم تسجيلهم في مستند واحد site_stats/counters عن طريق FieldValue.increment
// =====================================================================
document.addEventListener("DOMContentLoaded", function() {
    const whatsappLinks = document.querySelectorAll(".track-whatsapp-click");
    const callLinks = document.querySelectorAll(".track-call-click");
    if (!whatsappLinks.length && !callLinks.length) return;
    initFirebase();
    if (!db) return;

    whatsappLinks.forEach(function(el) {
        el.addEventListener("click", function() {
            incrementSiteCounter("whatsappClicks");
        });
    });
    callLinks.forEach(function(el) {
        el.addEventListener("click", function() {
            incrementSiteCounter("callClicks");
        });
    });
});

function incrementSiteCounter(field) {
...
// =====================================================================
// === عدادات بسيطة في لوحة التحكم الرئيسية (admin-home) ===
// عدد رسائل تواصل معنا / ضغطات واتساب / ضغطات اتصل بنا
// =====================================================================
document.addEventListener("DOMContentLoaded", function() {
    if (currentPage !== "admin-faq") return;
    loadAdminHomeStats();
    const resetBtn = document.getElementById("resetStatsBtn");
    if (resetBtn) resetBtn.addEventListener("click", resetSiteStats);
});

function loadAdminHomeStats() {
    const statBoxesExist = document.getElementById("statContactMessages");
    if (!statBoxesExist || !db) return;

    db.collection("contact_messages").get().then(function(snap) {
        const el = document.getElementById("statContactMessages");
        if (el) el.textContent = snap.size;
    }).catch(console.error);

    db.collection("site_stats").doc("counters").get().then(function(doc) {
        const data = doc.exists ? doc.data() : {};
        const waEl = document.getElementById("statWhatsappClicks");
        const callEl = document.getElementById("statCallClicks");
        if (waEl) waEl.textContent = data.whatsappClicks || 0;
        if (callEl) callEl.textContent = data.callClicks || 0;
    }).catch(console.error);
}

// إعادة ضبط عدادات الضغطات فقط (واتساب / اتصل بنا) - مش بيمسح رسائل تواصل معنا
function resetSiteStats() {
    if (!db) return;
    if (!confirm("متأكد إنك عايز تصفّر عداد ضغطات واتساب واتصل بنا؟ (الرسائل نفسها مش هتتمسح)")) return;

    db.collection("site_stats").doc("counters").set(
        { whatsappClicks: 0, callClicks: 0 },
        { merge: true }
    ).then(function() {
        ...
        console.error(e);
        showToast("حصل خطأ أثناء التصفير");
    });
}
